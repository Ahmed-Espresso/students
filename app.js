                            ...
                            <form id="editStudentForm" class="form-container">
                                ...
                                <div class="form-row">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" id="editParentName" readonly>
                                    </div>
                                </div>
        ...
        .then((snapshot) => {
            ...
            snapshot.forEach((childSnapshot) => {
                ...
                if (userId === selectedParentId) {
                    ...
                    if (selectedPhone) {
                        document.getElementById('editParentPhone').value = selectedPhone;
                    }
                }
...
function updateEditParentInfo() {
    ...
    if (parentSelect && parentNameInput && parentIdInput) {
        const selectedOption = parentSelect.options[parentSelect.selectedIndex];
        ...
        if (parentSelect.value) {
            ...
            // تحديث رقم الهاتف إذا كان فارغاً
            if (selectedOption.dataset.phone && !document.getElementById('editParentPhone').value) {
                document.getElementById('editParentPhone').value = selectedOption.dataset.phone;
            }
        } else {
            parentNameInput.value = '';
            parentIdInput.value = '';
                            ...
                            <form id="editUserForm" class="form-container">
                                ...
                                <div class="form-group">
                                    ...
                                    <select id="editRole" required>
                                        <option value="" ${!user.role ? 'selected' : ''}>اختر الدور</option>
...
function updateUser(userId) {
    ...
    const updatePromise = database.ref('users/' + userId).update(userData);
    
    // إذا كان هناك كلمة مرور جديدة، قم بتحديثها
    const authPromise = new Promise((resolve, reject) => {
        if (newPassword) {
            ...
            // جلب المستخدم من المصادقة وتحديث كلمة المرور
            firebase.auth().getUserByEmail(userData.email)
                .then((userRecord) => {
    ...
    Promise.all([updatePromise, authPromise])
        .then(() => {
